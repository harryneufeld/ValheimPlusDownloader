...
namespace GitHub.ReleaseDownloader
{
    public class ReleaseDownloader : IReleaseDownloader
    {
        ...
        string IReleaseDownloader.LatestVersion { get => this._latestVersion; }
        ...
        public bool IsLatestRelease(string version)
        {
            ...
            this._latestVersion = latestRelease.Value.ToString();
        ...
        public async Task<bool> DownloadLatestRelease()
        {
            ...
            List<string> assetUrls = await GetAssetUrlsAsync(latestReleaseId);
        ...
        private async Task<Dictionary<string, SemVersion>> GetReleasesAsync()
        {
            ...
            {
                var response = await _settings.HTTPClient.GetAsync(new Uri(_releasesEndpoint + "?page=" + pageNumber)).ConfigureAwait(false);
                var contentJson = await response.Content.ReadAsStringAsync();
                VerifyGitHubAPIResponse(response.StatusCode, contentJson);
                var jsonSerializer = new JavaScriptSerializer();
